...
class ezgal(object):
    ...
    def get_solar_rest_mags(self, nzs=1, filters=None, ab=None, vega=None, squeeze=True):
        ...
        for (i,filt) in enumerate(filters):
            ...
            if not filt in self.filters: raise ValueError('The filter %s could not be loaded!' % filt)
    ...
    def set_normalization(self, filter, z, mag, vega=False, apparent=False):
        ...
        if not filter in self.filters: self.add_filter(filter)
    ...
    def _grid_filters(self, filters=None, zfs=None, force=False, solar=False, masses=False):
        ...
        for filter in filters:
            ...
            if not filter in self.filters:
                ...
                if not filter in self.filters: raise ValueError('The specified filter, %s, has not been loaded and cannot be found!' % filter)
    ...
    def get_age(self, z1, z2, units='gyrs'):
        ...
        if type(z2) == list or type(z2) == np.ndarray:
    ...
    def extract_filters(self, filename, filters=None, grid=True):
        ...
        if not('nfilters' in fits[0].header): raise ValueError('Cannot extract filters from specified file because it has none!')
    ...
    def add_filter(self, file, name=None, units='a', grid=True):
        ...
        if name is None:
            if type(file) != str: raise ValueError('You need to pass a file name or a numpy array and filter name!')
    ...
    def save_model(self, model_file, filter_info=True, filter_only=False):
        ...
        if not filter_only:
            ...
            vs_hdu = pyfits.BinTableHDU.from_columns(pyfits.ColDefs([pyfits.Column(name='vs', array=self.vs, format='D', unit='hertz')]))
    ...
    def set_meta_data(self, data):
        ...
        if type(data) != dict or len(data.keys()) == 0: raise ValueError('Please pass a dictionary of meta data')
    ...
    #################################
    ## make exponential truncation ##
    #################################
    def make_exponential_truncation(self, tau, t_cut, tau_cut, dust_function=None, dust_args=(), max_err=0.001, max_iter=200):
        """ new_model = model.make_exponential_truncation(tau, dust_function=None, dust_args=(), max_err=0.001, max_iter=200)
        
        :param tau: Time scale for exponentially decaying burst of star formation (in gyrs).
        :param t_cut: Time of truncation (in gyrs).
        :param tau_cut: Time scale for truncation decay (in gyrs).
        :param dust_function: The dust dimming factor as a function of age and wavelength
        :param dust_args: A tuple with additional arguments to pass to dust_function
        :param max_err: Maximum allowed integration error (in magnitudes)
        :param max_iter: Maximum number of interations allowed when trying to get errors below ``max_err``
        ...
        :Example:
            ...
            >>> model = ezgal.model('bc03_ssp_z_0.02_chab.model')
            >>> # dust free exponentially decaying burst with tau=1 gyr
            >>> exp_1 = model.make_exponential_truncation(1.0)
        
        Returns a new EzGal object which exponentially decaying SFH with
        a timescale of ``tau`` and truncation at ``t_cut`` with
        timescale of ``tau_cut``.
        
        .. seealso::
            See :meth:`ezgal.ezgal.make_csp` for description of other parameters.
        """

        return self.make_csp(sfhs.exponential_truncation, args=(tau, t_cut, tau_cut), meta_data={'sfh': 'Truncation', 'tau': str(tau), 't_cut': str(t_cut), 'tau_cut': str(tau_cut)}, dust_function=dust_function, dust_args=dust_args, max_err=max_err, max_iter=max_iter)
    ...
    def _return_new(self, seds, masses=None, sfh=None, meta_data=None, weight=1):
        ...
        # meta data
        items = []
        if self.has_meta_data: items += list(self.meta_data.items())
        # do new meta data last so that it overwrites old meta data
        if meta_data is not None and type(meta_data) == dict: items += list(meta_data.items())
...
if __name__ == '__main__':
    ...
    for arg in args[1:]:
        ...
        if not(arg in model.filters) and not(os.path.isfile(arg)): raise ValueError('Could not load the filter %s!' % arg)
        filters.append(arg)
        nfilters += 1
    ...
    if group == 'filter' and out != 'all' and out != 'dm':
        ...
        if group == 'filter':
                    print('Ignoring group by filter specification - that only works for single column ouput!\n')
