...
if len( sys.argv ) < 3:
	...
	print('Usage: convert_conroy.py csp_model.out.spec csp_model.out\n')
	sys.exit(2)
...
while True:
	...
	# first data line contains nages and nls
	# second data line contains wavelengths
	if not c:
		[ nages, nls ] = descr.strip().split()
		( nages, nls ) = ( int( nages ), int( nls ) )
		ls = np.array( fp.readline().strip().split() ).astype( 'float' )
		c += 1
		continue
	...
	# now get sed data
	data = np.array( fp.readline().strip().split() ).reshape( (nls,1) ).astype( 'float' )
...
conv = 3.826e33/(4.0*math.pi*utils.convert_length( 10, incoming='pc', outgoing='cm' )**2.0)
vs = utils.convert_length( utils.c, incoming='m', outgoing='a' )/ls

# convert from Lo/Hz to ergs/s/Hz
seds *= conv
