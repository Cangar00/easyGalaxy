...
# convert to float and return original shape (m, 4)
arr = np.array( res ).reshape( (-1,4) ).astype( 'float' )
# extract age, lambda, and sed
arr = arr[:,[0,2,3]]
# grab unique ages
...
if filemass:
	...
	mass_interp = interpolate.interp1d( data[:,1]*1e9, data[:,2] )
...
if sfh and met and imf:
	...
	if hb: primary_hdu.header.update( 'hb', hb, comment='meta data' )
