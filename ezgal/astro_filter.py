...
class astro_filter(object):
    ...
    def __init__(self, filename, units='a', cosmology=None, vega=False, solar=False):
        ...
        if type(vega) == np.ndarray: self.set_vega_conversion(vega)

        # calculate solar magnitude if solar spectrum was passed
        if type(solar) == np.ndarray: self.set_solar_magnitude(solar)
    ...
    def calc_filter_properties(self):
        ...
        # mean wavelength
        self.mean = np.exp((self.tran_ls[m]*np.log(self.ls[m])*diff[m]/self.ls[m]).sum() / (self.tran_ls[m]*diff[m]/self.ls[m]).sum())
        # pivot wavelength
        self.pivot = np.sqrt((self.tran_ls[m]*self.ls[m]*diff[m]).sum()/(self.tran_ls[m]*diff[m]/self.ls[m]).sum())
        # average wavelength
        self.average = (self.tran_ls[m]*self.ls[m]*diff[m]).sum()/(self.tran_ls[m]*diff[m]).sum()
        # effective dimensionless gaussian width
        self.sig = np.sqrt((self.tran_ls[m]*np.log(self.ls[m]/self.mean)**2.0*diff[m]/self.ls[m]).sum() / (self.tran_ls[m]*diff[m]/self.ls[m]).sum())
        # effective width
    ...
    def set_vega_conversion(self, vega):
        ...
        if type(vega) != np.ndarray or vega.size == 0: raise ValueError('Pass a numpy array with the vega spectrum to calculate vega conversion!')

        # does the vega spectrum extend out to this filter?
        if self.vs.min() < vega[:,0].min() or self.vs.max() > vega[:,0].max(): return False
    ...
    def set_solar_magnitude(self, solar):
        ...
        if type(solar) != np.ndarray or solar.size == 0: raise ValueError('Pass a numpy array with the solar spectrum to calculate the solar magnitude!')

        # does the solar spectrum extend out to this filter?
        if self.vs.min() < solar[:,0].min() or self.vs.max() > solar[:,0].max():
            self.solar = np.nan
            return False
    ...
    def get_kcorrects(self, zf, zs):
        ...
        zf_grid = self.get_zf_grid(zf)
        if zf_grid == False: raise ValueError('Cannot fetch k-correction for given formation redshift because it has not been gridded!')
    ...
    def get_ecorrects(self, zf, zs):
        ...
        zf_grid = self.get_zf_grid(zf)
        if zf_grid == False: raise ValueError('Cannot fetch e-correction for given formation redshift because it has not been gridded!')
    ...
    def get_ekcorrects(self, zf, zs):
        ...
        zf_grid = self.get_zf_grid(zf)
        if zf_grid == False: raise ValueError('Cannot fetch e+k correction for given formation redshift because it has not been gridded!')
    ...
    def get_solar_mags(self, zf, zs, vega=False):
        ...
        zf_grid = self.get_zf_grid(zf)
        if zf_grid == False or not zf_grid.has_solar: raise ValueError('Cannot fetch solar magnitudes for given formation redshift because it has not been gridded!')
    ...
    def get_masses(self, zf, zs):
        ...
        zf_grid = self.get_zf_grid(zf)
        if zf_grid == False or not zf_grid.has_masses: raise ValueError('Cannot fetch masses for given formation redshift because it has not been gridded!')
    ...
    def calc_dm(self, zs):
        ...
        for i in range(zs.size): dms[i] = self.cosmo.DistMod(zs[i])
    ...
    def calc_mag(self, vs, sed, z):
        ...
        if shifted.min() > self.vs.min() or shifted.max() < self.vs.max(): return np.nan

        interp = interpolate.interp1d(vs, sed)
        sed_flux = (1+z)*scipy.integrate.simps(interp(self.vs*(1+z))*self.tran/self.vs, self.vs)
    ...
    def grid_solar(self, zf, solar_vs, solar_sed):
        ...
        if not self.has_zf(zf): raise ValueError('Cannot grid solar observed mags because the formation redshift has not been gridded yet!')
        ...
        for (i,z) in enumerate(zs): mags[i] = self.calc_mag(solar_vs, solar_sed, z)
    ...
    def grid_masses(self, zf, ages, masses):
        ...
        # This assumes that the given zf is already gridded - otherwise there is nothing to do
        if not self.has_zf(zf): raise ValueError('Cannot grid masses because the formation redshift has not been gridded yet!')
    ...
    ## store grid ##
    ################
    def store_grid(self, zf, zs, ages, rest, obs, dms=None, solar=None, masses=None):
