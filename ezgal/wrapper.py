...
class wrapper(object):
    ...
    def __init__(self, models, extra_data={}, extra_name=''):
        ...
        Initialize a wrapper object for handling multiple ezgal objects at once.
        Model list should be a list of ezgal objects or filenames of ezgal compatible model files.
    ...
    def __iadd__(self, more):
        if type(more) != type(self): raise TypeError('EzGal wrappers can only be added with each other!')
    ...
    def add_model(self, model, extra_data={}, extra_name=''):
        ...
        Add an ezgal object to the wrapper object.
        ``model`` can be an ezgal model object or the filename of an ezgal compatible model file."""

        # load the model if it is a filename
        if type(model) == str: model = ezgal.ezgal(model)
        ...
        if self.nmodels == 0:
            ...
            if (self.sed_shape != model.seds.shape or
                    self.sed_ages.size != model.ages.size or
                    np.max(np.abs(self.sed_ages - model.ages)) > 1e-8 or
                    self.sed_ls.size != model.ls.size or
                    np.max(np.abs(self.sed_ls - model.ls)) > 1e-8):
        ...
        this_meta_keys = list(extra_data.keys())
        this_meta_vals = list(extra_data.values())
        if model.has_meta_data:
            this_meta_keys += list(model.meta_data.keys())
            this_meta_vals += list(model.meta_data.values())
            extra_data.update(model.meta_data)
    ...
    def _normalize_data(self, extra_data, extra_name, return_list=False, require_length=False):
        ...
        if type(extra_name) != str or not extra_name:
            raise ValueError('When passing extra data a name must be provided if a list or scalar is passed')
        # require a list?
        if return_list and type(extra_data) != list: extra_data = [extra_data]
        # make a dictionary
    ...
    def find(self, keys, vals):
        """ model = wrapper_obj.find(keys, vals)
        ...
        if type(keys) != list and type(keys) != np.ndarray:
            keys = [keys]
    ...
    def interpolate(self, key, values, return_wrapper=True):
        ...
        if not key in self.meta_data: raise ValueError("Can't interpolate by %s because that key wasn't found in the wrapper object's meta data!" % key)
